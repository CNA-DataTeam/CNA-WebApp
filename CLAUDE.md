...
8. **MANDATORY: Before ANY git commit or push, rebuild the .exe AND run the config encrypt command.**
   These two steps must ALWAYS be run in this order before any git operation:
   1. Rebuild the exe: `RebuildExe.bat`
   2. Encrypt config: `.venv\Scripts\python.exe "CODE - do not open\config_manager.py" encrypt`
   Then stage and commit both `CNA Web App.exe` and `config.enc` alongside your other changes.
   This applies even if the user does not mention rebuilding or encryption — assume both are always needed unless the user explicitly says to skip them. Many users of this project are not aware of these technical requirements, so it is YOUR responsibility to always run both steps before any git operation. Never commit `config.py` directly — the repo is public.
...
1. Rebuild the exe: `RebuildExe.bat`
2. Encrypt config: `.venv\Scripts\python.exe "CODE - do not open\config_manager.py" encrypt`
3. Stage both `CNA Web App.exe` and `config.enc` alongside your other changes
4. Commit and push

**CRITICAL: Always run BOTH the exe rebuild and the encrypt command before any git commit or push. These are mandatory steps even if the user does not ask for them. The repo is public — never commit `config.py` directly.**
