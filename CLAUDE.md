...
8. **MANDATORY: Before ANY git commit or push, run the config encrypt command.**
   Run: `.venv\Scripts\python.exe "CODE - do not open\config_manager.py" encrypt`
   Then stage and commit `config.enc` alongside your other changes.
   This applies even if the user does not mention encryption — assume it is always needed unless the user explicitly says to skip it. Many users of this project are not aware of the technical requirement, so it is YOUR responsibility to always run this step before any git operation. Never commit `config.py` directly — the repo is public.
...
- `setup.bat`
  Creates `.venv`, installs dependencies, copies config key from network share, decrypts config.
- `StartApp.vbs`
  Hidden launcher wrapper. Runs `CODE - do not open\StartApp.bat` without a visible terminal window.
- `.venv/`
...
1. Run: `.venv\Scripts\python.exe "CODE - do not open\config_manager.py" encrypt`
2. Stage `config.enc` alongside your other changes
3. Commit and push

**CRITICAL: Always run the encrypt command before any git commit or push. This is a mandatory step even if the user does not ask for it. The repo is public — never commit `config.py` directly.**
...
### App launch
`StartApp.vbs` → `CODE - do not open/StartApp.bat`:
- `StartApp.vbs` is a VBScript wrapper that runs the batch file with no visible terminal window
- Sets `ROOT_DIR`, `CODE_DIR`, `APP_FILE`, `STARTUP_FILE`
- Adds root and code directories to `PYTHONPATH`
- If app is already running on port 8501, just opens the browser and exits
- Opens `splash.html` loading screen immediately in Edge app mode
- Copies `config.key` from network share if missing, then decrypts `config.enc` → `config.py`
- Launches `check_updates.py` in the background (checks git once per day, writes `.update_available` flag if behind)
- Runs `startup.py`
- Starts Streamlit with `pythonw -m streamlit run ... --server.port=8501`
- Splash screen auto-detects when Streamlit is ready and redirects
- Opens in Microsoft Edge app mode (`msedge --app=...`) with an isolated user profile
- Monitors the Edge process; when the window is closed, kills the Streamlit server
